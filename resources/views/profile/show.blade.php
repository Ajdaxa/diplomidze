...
@section('content')
    <div class="mx-auto w-full max-w-3xl">
        <h1 class="mb-2 text-[clamp(1.5rem,4vw,1.875rem)] font-semibold">Профиль</h1>
        <p class="mb-8 max-w-prose text-sm text-stone-500 sm:text-[0.9375rem]">Данные аккаунта и история заказов</p>

        <div class="mb-8 rounded-2xl border border-stone-200 bg-white p-4 sm:mb-10 sm:p-6">
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-5">
                @csrf
                <button type="submit" class="inline-flex min-h-10 items-center rounded-lg border border-stone-300 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-stone-700 hover:border-stone-900 hover:text-black">
                    Выйти из аккаунта
                </button>
            </form>
        </div>
        ...
        <div class="space-y-4">
            @forelse($user->clientOrders as $order)
                <div class="rounded-2xl border border-stone-200 bg-white p-4 sm:p-6">
